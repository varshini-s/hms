...
class Selection2 implements ActionListener
{
	JPanel Rooms;
	JLabel Select;
	JButton room[]=new JButton[16];
	...
	public void DrawRooms(String Hostel,String Block,int Floor,String Type,int RegNo)
	{
		...
		try{
			...
			for(int i=0;i<8;i++)
			{
				if(i<4)
				{
					room[i*2].setBounds(x,y,80,20);
					room[i*2+1].setBounds(x,y+20,80,20);
					x+=100;
				}
				else
				{
					room[i*2].setBounds(x1,y1,80,20);
					room[i*2+1].setBounds(x1,y1-20,80,20);
					x1+=100;
				}
